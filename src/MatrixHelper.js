export const matmul = (a, b) => {
  ...
  if (a.length === 0 || b.length === 0) {
    console.log(`Cannot multiply empty arrays. length a: ${a.length} length b: ${b.length}`)
    return
  }
  if (a[0].length !== b.length) {
    console.log(`The number of columns of a (${a[0].length}) must match the number of rows of b (${b.length}).`)
    return
  }
  return a.map(rowA => {
