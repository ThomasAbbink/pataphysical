...
export default (p) => {
  ...
  this.xSpeed = randomStartVal()
  this.ySpeed = randomStartVal()
  this.zSpeed = randomStartVal()
  this.wSpeed = randomStartVal()
  ...
  const createSliders = () => {
    ...
    this.xSlider.position(sliderPaddingLeft, p.windowHeight - (4 * sliderDividerHeight))

    this.ySlider = p.createSlider(0, 0.05, this.ySpeed, 0.001)
    this.ySlider.position(sliderPaddingLeft, p.windowHeight - (3 * sliderDividerHeight))

    this.zSlider = p.createSlider(0, 0.05, this.zSpeed, 0.001)
    this.zSlider.position(sliderPaddingLeft, (p.windowHeight) - (2 * sliderDividerHeight))

    this.wSlider = p.createSlider(0, 0.05, this.wSpeed, 0.001)
    this.wSlider.position(sliderPaddingLeft, p.windowHeight - sliderDividerHeight)
  }
  ...
  p.draw = () => {
    ...
    p.noStroke()
    p.textSize(30)
    p.text(`x: ${this.xSlider.value()}`, this.xSlider.width + (this.xSlider.x * 2), this.xSlider.y + (this.xSlider.height / 2))
    p.text(`y: ${this.ySlider.value()}`, this.ySlider.width + (this.ySlider.x * 2), this.ySlider.y + (this.ySlider.height / 2))
    p.text(`z: ${this.zSlider.value()}`, this.zSlider.width + (this.zSlider.x * 2), this.zSlider.y + (this.zSlider.height / 2))
    p.text(`w: ${this.wSlider.value()}`, this.wSlider.width + (this.wSlider.x * 2), this.wSlider.y + (this.wSlider.height / 2))
